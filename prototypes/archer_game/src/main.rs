...
impl Game {
    fn fire_arrow(&mut self) {
        if self.user_inputs.va_space {
            self.add_things(vec![(
                (100_f32, 100_f32),
                (Vector::new(1_f32, 0_f32), 0.25, 2_u32),
            )])
        }
    }
    pub fn new() -> Game {
    ...
    pub fn add_things(&mut self, new_things_list: Vec<((f32, f32), (Vector, f32, u32))>) {
        for thing in new_things_list.into_iter() {
            ...
            self.position_components
                .add(PositionComponent::new(pos_x, pos_y));
            self.projectile_components
                .add(ProjectileComponent::new(pos_x, pos_y));

            self.renderable_components
                .add(RenderableComponent::new(None, facing, size, texture));
        }
        self.renderable_components.update_all_coords(
            &self.view_area,
            &self.position_components,
            &self.projectile_components,
        );
    ...
    fn update(&mut self) {
        self.position_components
            .update_using_input(&self.user_inputs);
        self.projectile_components
            .update_using_input(&self.user_inputs);
...
impl ViewArea {
    ...
    fn pos_cords_to_render_cords(&self, thing_position: &PositionComponent) -> (f32, f32) {
        let potential_render_x: f32 =
            (thing_position.x - self.position.x) / (ViewArea::LENGTH / 2_f32);
        let potential_render_y: f32 =
            -(thing_position.y - self.position.y) / (ViewArea::WIDTH / 2_f32);
...
impl PositionComponentVec {
    ...
    fn update_using_input(&mut self, user_inputs: &UserInputs) {
        ...
        if movement_vec.length() != 0_f32 {
            movement_vec = movement_vec
                .normalize()
                .scale_by(PositionComponentVec::PLAYERSPEED);
        }
    ...
    fn new() -> PositionComponentVec {
        PositionComponentVec { parts: Vec::new() }
    }
    ...
    fn get_render_positions(&self, view_area: &ViewArea) -> Vec<Vector> {
        ...
        for i in 0..self.parts.len() {
            let (render_x, render_y): (f32, f32) =
                view_area.pos_cords_to_render_cords(&self.parts[i]);
...
impl ProjectileComponentVec {
    ...
    fn update_using_input(&mut self, user_inputs: &UserInputs) {
        ...
        if movement_vec.length() != 0_f32 {
            movement_vec = movement_vec
                .normalize()
                .scale_by(ProjectileComponentVec::ARROWSPEED);
        }
    ...
    fn new() -> ProjectileComponentVec {
        ProjectileComponentVec { parts: Vec::new() }
    }
    ...
    fn get_render_positions(&self, view_area: &ViewArea) -> Vec<Vector> {
        let mut render_positions: Vec<Vector> = Vec::new();
        let (render_x, render_y): (f32, f32) =
            view_area.arrow_pos_cords_to_render_cords(&self.parts[257]);
        render_positions.push(Vector::new(render_x, render_y));
        //println!("{:?}", render_positions);
        render_positions
    }
...
impl RenderableComponentVec {
    fn new() -> RenderableComponentVec {
        RenderableComponentVec { parts: Vec::new() }
    }
    fn add(&mut self, new_component: RenderableComponent) {
        self.parts.push(new_component);
    }
    fn update_all_coords(
        &mut self,
        view_area: &ViewArea,
        position_component_vec: &PositionComponentVec,
        projectile_component_vec: &ProjectileComponentVec,
    ) {
    ...
    fn render_all(&self) -> (Vec<Vertex>, Vec<u16>) {
        ...
        for thing in self.parts.iter() {
            let potential_placeholder: Option<(Vec<[f32; 3]>, Vec<[f32; 2]>, Vec<u16>)> =
                thing.render();
            match potential_placeholder {
                Some(i) => {
                    ...
                    let combined_vertices: Vec<([f32; 3], [f32; 2])> = placeholder
                        .0
                        .into_iter()
...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        let (diffuse_texture, cmd_buffer) =
            texture::Texture::from_bytes(&device, diffuse_bytes, "texture.png").unwrap();
        ...
        match event {
            WindowEvent::KeyboardInput {
                ...
                match virtual_key_code {
                    VirtualKeyCode::W => {
                        self.game.user_inputs.va_up = *state == ElementState::Pressed
                    }
                    VirtualKeyCode::S => {
                        self.game.user_inputs.va_down = *state == ElementState::Pressed
                    }
                    VirtualKeyCode::A => {
                        self.game.user_inputs.va_left = *state == ElementState::Pressed
                    }
                    VirtualKeyCode::D => {
                        self.game.user_inputs.va_right = *state == ElementState::Pressed
                    }
                    VirtualKeyCode::Space => {
                        self.game.user_inputs.va_space = *state == ElementState::Pressed
                    }
                    _ => (),
                }
                true
